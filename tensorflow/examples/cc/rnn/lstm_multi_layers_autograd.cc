...
int main() { 
  ...
  TF_CHECK_OK(AddSymbolicGradients(root, {rnn_softmax_loss.loss}, 
                                   {w_layer1, b_layer1, w_layer2, b_layer2, w_y, b_y}, 
                                   &grad_outputs));
  LOG(INFO) << "Node building status: " << root.status();
  ...
  auto apply_w_layer1 = ApplyAdagradTrick(root, w_layer1, ada_w_layer1, lr, grad_outputs[0]);
  auto apply_b_layer1 = ApplyAdagradTrick(root, b_layer1, ada_b_layer1, lr, grad_outputs[1]);
  auto apply_w_layer2 = ApplyAdagradTrick(root, w_layer2, ada_w_layer2, lr, grad_outputs[2]);
  auto apply_b_layer2 = ApplyAdagradTrick(root, b_layer2, ada_b_layer2, lr, grad_outputs[3]);
  auto apply_w_y = ApplyAdagradTrick(root, w_y, ada_w_y, lr, grad_outputs[4]); // only apply on output layer
  auto apply_b_y = ApplyAdagradTrick(root, b_y, ada_b_y, lr, grad_outputs[5]); // only apply on output layer
  ...
  auto block_lstm_eval_layer2 = BlockLSTM(root, 
                              ...
                              Const<float>(root, 0, TensorShape({NUM_UNIT})), // wco
                              b_layer2);
  auto rnn_softmax_loss_eval = RNNSoftmaxLoss(root, 
  ...
  TF_CHECK_OK(session.Run({assign_w_layer1, assign_b_layer1, assign_w_layer2, assign_b_layer2, assign_w_y, assign_b_y, assign_cs}, 
                          nullptr));
