...
int main() { 
  ...
  auto w = Variable(root, {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT); // (input_size + cell_size, cell_size * 4) for {i, c, f, o}
  ...
  auto block_lstm = BlockLSTM(root, 
                              ...
                              b);
  auto rnn_softmax_loss_hgrad = RNNSoftmaxLossHGrad(root, 
                              block_lstm.h,
                              y,
                              w_y,
                              b_y);
  auto block_lstm_grad = BlockLSTMGrad(root,
                              Const<int64>(root, {TIME_LEN}), // seq_len_max,
                              x, 
                              cs_prev,
                              h_prev,
  ...
  while(step < TRAINING_STEPS) {
    ...
    if(step % 100 == 0) 
    {
        ...
        for(int i = 0; i < 20; i++) 
        {
          ...
          TF_CHECK_OK(session.Run({{x_eval, x_tensor}, {y_eval, y_tensor}, 
                                              {h_prev_eval, eval_h_prev_tensor}, {cs_prev_eval, eval_cs_prev_tensor}}, 
          ...
          CHECK(eval_cs_prev_tensor.CopyFrom(outputs[3].Slice(0, 1), {outputs[3].dim_size(1), outputs[3].dim_size(2)}));
        } // for(int i = 0; i < 20; i++) {
    } // Evaluate
    ...
    // Train
    {
      ...
      TF_CHECK_OK(session.Run({{x, x_tensor}, {y, y_tensor}, {h_prev, h_prev_tensor}, {cs_prev, cs_prev_tensor}, 
                              ...
                              {rnn_softmax_loss_hgrad.loss, 
                                              block_lstm_grad.h_prev_grad, block_lstm_grad.cs_prev_grad, 
