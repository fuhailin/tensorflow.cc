...
// Grad
template <typename T, typename Index>
struct GradFunctor<GPUDevice, T, Index> {
  void operator()(OpKernelContext* ctx, const GPUDevice& d, typename TTypes<T>::ConstMatrix h,
...
#define REGISTER(Index)                                                      \
  ...
  template struct functor::GradFunctor<GPUDevice, float, Index>;       \
  template class generator::RNNSoftmaxLossGradGenerator<float, Index>;           \
  template struct functor::SparseXentFunctor<GPUDevice, Eigen::half, Index>; \
  template struct functor::GradFunctor<GPUDevice, Eigen::half, Index>; \
  template class generator::RNNSoftmaxLossGradGenerator<Eigen::half, Index>;
