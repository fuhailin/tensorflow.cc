...
// #endif
  ...
  for(int i = 0; i < GENERATE_STEPS; i++) {
    ...
    Status run_status = clientSession.Run(RunOptions(), {{input_layer, x_tensor}, {label_layer, y_tensor}, {h_prev_layer, h_prev_tensor}},
    ...
    run_status = clientSession.Run(RunOptions(), {{topk_input_layer, topk_input_tensor}},
