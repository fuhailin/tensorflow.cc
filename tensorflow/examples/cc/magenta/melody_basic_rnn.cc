...
int main() {
  ...
  gtl::ArraySlice<PartialTensorShape> feature_list_dense_shapes = {{}, {VOCAB_SIZE}};
  ...
  feature_list_dense_defaults.push_back(Const<float>(root, 1, TensorShape({VOCAB_SIZE})));
  ...
  // Placeholders
  auto x = Placeholder(root, DT_FLOAT, Placeholder::Shape({SEQ_LENGTH, VOCAB_SIZE, 1}));
  auto y = Placeholder(root, DT_FLOAT, Placeholder::Shape({SEQ_LENGTH}));
  auto h_prev = Placeholder(root, DT_FLOAT, Placeholder::Shape({HIDDEN_SIZE, 1}));
  ...
  Output apply_w_xh = ApplyAdagradTrick(root, w_xh, ada_w_xh, lr, vanilla_rnn_grad.d_w_xh);
  Output apply_w_hh = ApplyAdagradTrick(root, w_hh, ada_w_hh, lr, vanilla_rnn_grad.d_w_hh);
  Output apply_w_hy = ApplyAdagradTrick(root, w_hy, ada_w_hy, lr, vanilla_rnn_grad.d_w_hy);
  Output apply_b_h = ApplyAdagradTrick(root, b_h, ada_b_h, lr, vanilla_rnn_grad.d_b_h);
  Output apply_b_y = ApplyAdagradTrick(root, b_y, ada_b_y, lr, vanilla_rnn_grad.d_b_y);
  ...
  // #ifdef VERBOSE  
        LOG(INFO) << __FUNCTION__ << "----------------Evaluate index: " << index;  
        // LOG(INFO) << __FUNCTION__ << "----------------Evaluate mat: " << mat;
  // #endif
      ...
      // Train
      {
        ...
        for (int i = 0; i < SEQ_LENGTH; i++) {
          y_t(i) = static_cast<float>(dataset_outputs[0].vec<int64>()(y_index++));
        }
        ...
        TF_CHECK_OK(session.Run({{x, x_tensor}, {y, y_tensor}, {h_prev, h_prev_tensor}}, 
                                {vanilla_rnn.h, vanilla_rnn.loss,
                                  apply_w_xh, apply_w_hh, apply_w_hy, apply_b_h, apply_b_y}, 
                                {}, 
                                &outputs));

        if(step % 1000 == 0)
          LOG(INFO) << "Print step: " << step << ", loss: " << outputs[1].DebugString();
