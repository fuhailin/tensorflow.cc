...
#define DIR 1                    // For now, DIR must be 1, TODO.  // dir = (direction == bidirectional) ? 2 : 1
...
int main() { 
  ...
  for(int i = 0; i < num_gates; i++) {
    ...
    TF_CHECK_OK(session.Run({assign_v}, 
                          nullptr));
  ...
  auto crctp = CudnnRNNCanonicalToParams(root, 
                            NUM_LAYERS,
                            NUM_UNIT,
                            INPUT_SIZE,
                            InputList(weights),
  ...
  LOG(INFO) << "-------------------------------------CudnnRNNCanonicalToParams status: " << root.status();

  // params_size
  vector<Tensor> params_outputs;
  TF_CHECK_OK(session.Run({}, {crctp}, {}, &params_outputs));
  LOG(INFO) << "----------------------------params_outputs: " << params_outputs[0].DebugString();

  vector<Tensor> params_size_outputs;
  auto cudnn_RNN_params_size = CudnnRNNParamsSize(root, 
                                                  Const<int32>(root, NUM_LAYERS, TensorShape({})),      // num_layers, 
  ...
  LOG(INFO) << "-------------------------------------CudnnRNNParamsSize status: " << root.status();
  TF_CHECK_OK(session.Run({}, {cudnn_RNN_params_size}, {}, &params_size_outputs));
  int param_size = params_size_outputs[0].scalar<int32>()();
  LOG(INFO) << "----------------------------params_size_outputs: " << params_size_outputs[0].DebugString() << ", param_size: " << param_size;
  ...
  auto assign_params = Assign(root, params, params_outputs[0]);
    ...
    auto assign_params = Assign(root, params, ZerosLike(root, params_zero_tensor));
  #endif
#endif
  ...
  auto assign_ada_params = Assign(root, ada_params, ZerosLike(root, ada_params_zero_tensor));
  ...
  // Placeholders
  auto input = Placeholder(root, DT_FLOAT, Placeholder::Shape({TIME_LEN, BATCH_SIZE, INPUT_SIZE}));
  auto input_h = Placeholder(root, DT_FLOAT, Placeholder::Shape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
  auto input_c = Placeholder(root, DT_FLOAT, Placeholder::Shape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
  auto output_backprop = Placeholder(root, DT_FLOAT, Placeholder::Shape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));
  auto output_h_backprop = Placeholder(root, DT_FLOAT, Placeholder::Shape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT})); // [num_layer * dir, batch_size, num_units].
  auto output_c_backprop = Placeholder(root, DT_FLOAT, Placeholder::Shape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT})); // [num_layer * dir, batch, num_units]
  ...
  auto cudnn_RNN = CudnnRNN(root, 
                             ...
                             input_c,                                        // input_c,
                             params);
  LOG(INFO) << "-------------------------------------CudnnRNN status: " << root.status();
  auto rnn_softmax_loss_hgrad = RNNSoftmaxLossHGrad(root, 
                              cudnn_RNN.output_h, // [seq_length, batch_size, dir * num_units] {timelen, batch_size, cell_size aka. num_units}
                              y,
                              w_y,
                              b_y);
  LOG(INFO) << "-------------------------------------RNNSoftmaxLossHGrad status: " << root.status();
  auto cudnn_RNN_backprop = CudnnRNNBackprop(root, 
                                             input,
                                             input_h, 
                                             input_c,
                                             params, 
                                             ...
                                             cudnn_RNN.output_c,
                                             output_backprop,                                             
                                             output_h_backprop,  // ?? rnn_softmax_loss_hgrad.h_grad,  // [seq_length, batch_size, dir * num_units]
                                             output_c_backprop,
                                             cudnn_RNN.reserve_space);
  LOG(INFO) << "-------------------------------------CudnnRNNBackprop status: " << root.status();

  // Gradient
  auto lr = Cast(root, 0.03, DT_FLOAT);

  // alternative of ApplyAdagrad
  auto apply_params = ApplyAdagradTrick(root, params, ada_params, lr, cudnn_RNN_backprop.params_backprop);
  ...
  TF_CHECK_OK(session.Run({assign_params, assign_w_y, assign_b_y}, 
                          nullptr));
  ...
  while(step < TRAINING_STEPS) {
    ...
    Tensor input_h_tensor(DT_FLOAT, TensorShape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
    typename TTypes<float, 3>::Tensor input_h_t = input_h_tensor.tensor<float, 3>();
    input_h_t.setZero();

    Tensor input_c_tensor(DT_FLOAT, TensorShape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
    typename TTypes<float, 3>::Tensor input_c_t = input_c_tensor.tensor<float, 3>();
    input_c_t.setZero();

    Tensor output_backprop_tensor(DT_FLOAT, TensorShape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));
    typename TTypes<float, 3>::Tensor output_backprop_t = output_backprop_tensor.tensor<float, 3>();
    output_backprop_t.setZero();

    Tensor output_h_backprop_tensor(DT_FLOAT, TensorShape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
    typename TTypes<float, 3>::Tensor output_h_backprop_t = output_h_backprop_tensor.tensor<float, 3>();
    output_h_backprop_t.setZero();

    Tensor output_c_backprop_tensor(DT_FLOAT, TensorShape({NUM_LAYERS * DIR, BATCH_SIZE, NUM_UNIT}));
    typename TTypes<float, 3>::Tensor output_c_backprop_t = output_c_backprop_tensor.tensor<float, 3>();
    ...
    // Train
    {
      ...
      TF_CHECK_OK(session.Run({{input, x_tensor}, {y, y_tensor}, {input_h, input_h_tensor}, {input_c, input_c_tensor}, 
                               {output_backprop, output_backprop_tensor}, {output_h_backprop, output_h_backprop_tensor}, 
