...
int main() { 
  ...
  auto w_layer1 = Variable(root, {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT); // (input_size + cell_size, cell_size * 4) for {i, c, f, o}
  auto random_value = RandomNormal(root, {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  auto assign_w_layer1 = Assign(root, w_layer1, Multiply(root, random_value, rate));

  auto w_layer2 = Variable(root, {NUM_UNIT + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT); // (cell_size + cell_size, cell_size * 4) for {i, c, f, o}
  auto random_value21 = RandomNormal(root, {NUM_UNIT + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  auto assign_w_layer2 = Assign(root, w_layer2, Multiply(root, random_value21, rate));
  ...
  auto ada_w_layer1 = Variable(root, {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  auto assign_ada_w_layer1 = Assign(root, ada_w_layer1, ZerosLike(root, w_layer1));

  auto ada_w_layer2 = Variable(root, {NUM_UNIT + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  auto assign_ada_w_layer2 = Assign(root, ada_w_layer2, ZerosLike(root, w_layer2));

  auto ada_b_layer1 = Variable(root, {NUM_UNIT * 4}, DT_FLOAT);
  auto assign_ada_b_layer1 = Assign(root, ada_b_layer1, ZerosLike(root, b_layer1));

  auto ada_b_layer2 = Variable(root, {NUM_UNIT * 4}, DT_FLOAT);
  auto assign_ada_b_layer2 = Assign(root, ada_b_layer2, ZerosLike(root, b_layer2));
  ...
  auto cs_prev_layer1 = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT})); // (batch_size, cell_size)
  auto h_prev_layer1 = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT}));
  auto cs_grad_layer1 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));

  auto cs_prev_layer2 = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT})); // (batch_size, cell_size)
  auto h_prev_layer2 = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT}));
  auto cs_grad_layer2 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));

  // LSTM
  auto block_lstm_layer1 = BlockLSTM(root, 
                              ...
                              b_layer1);
  auto block_lstm_layer2 = BlockLSTM(root, 
                              Const<int64>(root, {TIME_LEN}),                 // seq_len_max,
                              ...
                              Const<float>(root, 0, TensorShape({NUM_UNIT})), // wco
                              b_layer2);
  auto rnn_softmax_loss_hgrad_layer2 = RNNSoftmaxLossHGrad(root, 
                              block_lstm_layer2.h,
                              y,
                              w_y,
                              b_y);
  auto block_lstm_grad_layer2 = BlockLSTMGrad(root,
                              Const<int64>(root, {TIME_LEN}),                 // seq_len_max,
                              block_lstm_layer1.h,
                              cs_prev_layer2,
                              h_prev_layer2,
                              ...
                              b_layer2,
                              block_lstm_layer2.i,
                              block_lstm_layer2.cs,
                              block_lstm_layer2.f,
                              block_lstm_layer2.o,
                              block_lstm_layer2.ci,
                              block_lstm_layer2.co,
                              block_lstm_layer2.h,
                              cs_grad_layer2,
                              rnn_softmax_loss_hgrad_layer2.h_grad,
                              false                                           // use_peephole
                              );
  auto block_lstm_grad_layer1 = BlockLSTMGrad(root,
                              Const<int64>(root, {TIME_LEN}),                 // seq_len_max,
                              x, 
                              cs_prev_layer1,
                              h_prev_layer1,
                              ...
                              b_layer1,
                              block_lstm_layer1.i,
                              block_lstm_layer1.cs,
                              block_lstm_layer1.f,
                              block_lstm_layer1.o,
                              block_lstm_layer1.ci,
                              block_lstm_layer1.co,
                              block_lstm_layer1.h,
                              cs_grad_layer1,
  ...
  auto apply_w_layer1 = ApplyAdagradTrick(root, w_layer1, ada_w_layer1, lr, block_lstm_grad_layer1.w_grad);
  auto apply_w_layer2 = ApplyAdagradTrick(root, w_layer2, ada_w_layer2, lr, block_lstm_grad_layer2.w_grad);
  auto apply_b_layer1 = ApplyAdagradTrick(root, b_layer1, ada_b_layer1, lr, block_lstm_grad_layer1.b_grad);
  auto apply_b_layer2 = ApplyAdagradTrick(root, b_layer2, ada_b_layer2, lr, block_lstm_grad_layer2.b_grad);
  auto apply_w_y = ApplyAdagradTrick(root, w_y, ada_w_y, lr, rnn_softmax_loss_hgrad_layer2.dw_y); // only apply on output layer
  auto apply_b_y = ApplyAdagradTrick(root, b_y, ada_b_y, lr, rnn_softmax_loss_hgrad_layer2.db_y); // only apply on output layer
  ...
  auto cs_prev_eval_layer1 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TEST_BATCH_SIZE, NUM_UNIT})); // (batch_size, cell_size)
  auto h_prev_eval_layer1 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TEST_BATCH_SIZE, NUM_UNIT}));
  auto cs_prev_eval_layer2 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TEST_BATCH_SIZE, NUM_UNIT})); // (batch_size, cell_size)
  auto h_prev_eval_layer2 = Placeholder(root, DT_FLOAT, Placeholder::Shape({TEST_BATCH_SIZE, NUM_UNIT}));
  // BlockLSTM
  auto block_lstm_eval_layer1 = BlockLSTM(root, 
                              Const<int64>(root, {TEST_TIME_LEN}), // seq_len_max,
                              x_eval,
                              cs_prev_eval_layer1,
                              h_prev_eval_layer1, 
                              w_layer1,
                              ...
                              Const<float>(root, 0, TensorShape({NUM_UNIT})), // wco
                              b_layer1);
  auto block_lstm_eval_layer2 = BlockLSTM(root, 
                              Const<int64>(root, {TEST_TIME_LEN}), // seq_len_max,
                              block_lstm_eval_layer1.h,
                              cs_prev_eval_layer2,
                              h_prev_eval_layer2, 
                              w_layer2,
                              Const<float>(root, 0, TensorShape({NUM_UNIT})), // wci, used when use_peephole is true
                              ...
                              Const<float>(root, 0, TensorShape({NUM_UNIT})), // wco
                              b_layer2);
  auto rnn_softmax_loss_hgrad_eval = RNNSoftmaxLossHGrad(root, 
                              block_lstm_eval_layer2.h,
                              y_eval,
                              w_y,
                              b_y);
  ...
  while(step < TRAINING_STEPS) {
    ...
    if(step % 100 == 0) 
    {
        ...
        tensorflow::Tensor eval_h_prev_layer1_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({TEST_BATCH_SIZE, NUM_UNIT}));
        typename TTypes<float>::Matrix eval_h_prev_layer1_t = eval_h_prev_layer1_tensor.matrix<float>();
        eval_h_prev_layer1_t.setZero();

        Tensor eval_cs_prev_layer1_tensor(DT_FLOAT, TensorShape({TEST_BATCH_SIZE, NUM_UNIT}));
        typename TTypes<float>::Matrix eval_cs_prev_layer1_t = eval_cs_prev_layer1_tensor.matrix<float>();
        eval_cs_prev_layer1_t.setZero();

        tensorflow::Tensor eval_h_prev_layer2_tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({TEST_BATCH_SIZE, NUM_UNIT}));
        typename TTypes<float>::Matrix eval_h_prev_layer2_t = eval_h_prev_layer2_tensor.matrix<float>();
        eval_h_prev_layer2_t.setZero();

        Tensor eval_cs_prev_layer2_tensor(DT_FLOAT, TensorShape({TEST_BATCH_SIZE, NUM_UNIT}));
        typename TTypes<float>::Matrix eval_cs_prev_layer2_t = eval_cs_prev_layer2_tensor.matrix<float>();
        ...
        for(int i = 0; i < 20; i++) 
        {
          ...
          // Run 
          TF_CHECK_OK(session.Run({{x_eval, x_tensor}, {y_eval, y_tensor}, 
                                              {h_prev_eval_layer1, eval_h_prev_layer1_tensor}, {cs_prev_eval_layer1, eval_cs_prev_layer1_tensor},
                                              {h_prev_eval_layer2, eval_h_prev_layer2_tensor}, {cs_prev_eval_layer2, eval_cs_prev_layer2_tensor}}, 
          ...
          CHECK(eval_h_prev_layer1_tensor.CopyFrom(outputs[2].Slice(0, 1), {outputs[2].dim_size(1), outputs[2].dim_size(2)}));
          CHECK(eval_cs_prev_layer1_tensor.CopyFrom(outputs[3].Slice(0, 1), {outputs[3].dim_size(1), outputs[3].dim_size(2)}));
          CHECK(eval_h_prev_layer2_tensor.CopyFrom(outputs[4].Slice(0, 1), {outputs[4].dim_size(1), outputs[4].dim_size(2)}));
          CHECK(eval_cs_prev_layer2_tensor.CopyFrom(outputs[5].Slice(0, 1), {outputs[5].dim_size(1), outputs[5].dim_size(2)}));
        }
    } // Evaluate
    ...
    // Train
    {
      ...
      Tensor h_prev_layer1_tensor(DT_FLOAT, TensorShape({BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float>::Matrix h_prev_layer1_t = h_prev_layer1_tensor.matrix<float>();
      h_prev_layer1_t.setZero();

      Tensor cs_prev_layer1_tensor(DT_FLOAT, TensorShape({BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float>::Matrix cs_prev_layer1_t = cs_prev_layer1_tensor.matrix<float>();
      cs_prev_layer1_t.setZero();

      // Always zero
      Tensor cs_grad_layer1_tensor(DT_FLOAT, TensorShape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float, 3>::Tensor cs_grad_layer1_t = cs_grad_layer1_tensor.tensor<float, 3>();
      cs_grad_layer1_t.setZero();

      Tensor h_prev_layer2_tensor(DT_FLOAT, TensorShape({BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float>::Matrix h_prev_layer2_t = h_prev_layer2_tensor.matrix<float>();
      h_prev_layer2_t.setZero();

      Tensor cs_prev_layer2_tensor(DT_FLOAT, TensorShape({BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float>::Matrix cs_prev_layer2_t = cs_prev_layer2_tensor.matrix<float>();
      cs_prev_layer2_t.setZero();

      // Always zero
      Tensor cs_grad_layer2_tensor(DT_FLOAT, TensorShape({TIME_LEN, BATCH_SIZE, NUM_UNIT}));
      typename TTypes<float, 3>::Tensor cs_grad_layer2_t = cs_grad_layer2_tensor.tensor<float, 3>();
      ...
      TF_CHECK_OK(session.Run({{x, x_tensor}, {y, y_tensor}, 
                                              {h_prev_layer1, h_prev_layer1_tensor}, {cs_prev_layer1, cs_prev_layer1_tensor}, {cs_grad_layer1, cs_grad_layer1_tensor},
                                              {h_prev_layer2, h_prev_layer2_tensor}, {cs_prev_layer2, cs_prev_layer2_tensor}, {cs_grad_layer2, cs_grad_layer2_tensor}}, 
                              {rnn_softmax_loss_hgrad_layer2.loss, 
                                              apply_w_layer1, apply_b_layer1, apply_w_layer2, apply_b_layer2, apply_w_y, apply_b_y}, 
                              {}, 
                              &outputs));
