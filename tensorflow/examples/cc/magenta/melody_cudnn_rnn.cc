...
#include "cudnn_rnn/cudnn_rnn_ops.h"
...
int main() { 
  ...
  int param_size = params_size_outputs[0].scalar<float>()();
  ...
  auto assign_ada_params = Assign(root, ada_params, ZerosLike(root, params));
  ...
  auto rnn_softmax_loss_hgrad = RNNSoftmaxLossHGrad(root, 
                              cudnn_RNN.output, // [seq_length, batch_size, dir * num_units]
                              y,
                              w_y,
                              b_y);
  ...
  auto cudnn_RNN_backprop = CudnnRNNBackprop(root, 
                                             ...
                                             cudnn_RNN.output_h,
                                             cudnn_RNN.output_c,
                                             rnn_softmax_loss_hgrad.h_grad,  // [seq_length, batch_size, dir * num_units]
                                             output_h_backprop, 
                                             output_c_backprop,
  ...
  auto apply_params = ApplyAdagradTrick(root, params, assign_ada_params, lr, cudnn_RNN_backprop.params_backprop);
  ...
  while(step < TRAINING_STEPS) {
    ...
    // Train
    {
      ...
      TF_CHECK_OK(session.Run({{input, x_tensor}, {y, y_tensor}, {input_h, input_h_tensor}, {input_c, input_c_tensor}, 
                                              {output_h_backprop, output_h_backprop_tensor}, {output_c_backprop, output_c_backprop_tensor}}, 
  ...
  // Update input_h_tensor and input_c_tensor, output_c_backprop_tensor for the next BATCH
