...
#ifndef TENSORFLOW_CORE_KERNELS_RNN_SOFTMAXLOSS_OP_H_
#define TENSORFLOW_CORE_KERNELS_RNN_SOFTMAXLOSS_OP_H_
...
// Modified by Rock Zhuang, June 01, 2019;
...
// for j = 0 .. num_classes.
template <typename T, typename Index>
class RNNSoftmaxLossGradGenerator {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE RNNSoftmaxLossGradGenerator(
      typename TTypes<const T, 2>::Tensor32Bit prob,
      typename TTypes<const Index, 1>::Tensor32Bit labels,
      const Index max_depth)
      : prob_(prob),
        labels_(labels),
  ...
  operator()(const Eigen::array<int, 2>& coords) const {
    ...
    return prob_(coords) - subtract;
  };

 private:
  typename TTypes<const T, 2>::Tensor32Bit prob_;
  typename TTypes<const Index, 1>::Tensor32Bit labels_;
  const Index max_depth_;
};
...
// Functor used by GradFunctor to do the computations.
template <typename Device, typename T, typename Index>
struct GradFunctor {
  // Computes Cross Entropy loss and backprop.
  //
  ...
  // loss: output tensor for the loss, dims: batch_size.
  // backprop: output tensor for the backprop, dims: batch_size, num_classes.
  void operator()(OpKernelContext* ctx, const Device& d, typename TTypes<T>::ConstMatrix h,
...
#endif  // TENSORFLOW_CORE_KERNELS_RNN_SOFTMAXLOSS_OP_H_
