...
class InternalClientSession {
 public:
  static tensorflow::Session* GetSession(tensorflow::ClientSession& session) {  // NOLINT(runtime/references)
...
int main() {
  ...
  //
  // The file 'training_melodies.tfrecord' was generated by running the following commands from Magenta project:
  // (put midi files into the folder '/tmp/data/midi/' in advance)
  //   $ convert_dir_to_note_sequences --input_dir=/tmp/data/midi/ --output_file=/tmp/data/notesequences.tfrecord --recursive            // NOLINT
  //   $ melody_rnn_create_dataset --config=basic_rnn --input=/tmp/data/notesequences.tfrecord --output_dir=/tmp/data/ --eval_ratio=0.0  // NOLINT
  //
  ...
  Output iterator_output = Iterator(root, "iterator1", "",
                                    vector<DataType>({DT_STRING}),
                                    vector<PartialTensorShape>({{}}));
  Operation make_iterator_op = MakeIterator(root, shuffle_and_repeat_dataset, iterator_output);
  auto iterator_get_next = IteratorGetNext(root, iterator_output,
                                           vector<DataType>({DT_STRING}),
                                           vector<PartialTensorShape>({{}}));
  ...
  auto parse_single_sequence_example = ParseSingleSequenceExample(root, iterator_get_next[0],
                            ...
                            ParseSingleSequenceExample::Attrs().FeatureListDenseTypes(feature_list_dense_types)
  ...
  auto enqueue = QueueEnqueue(root.WithOpName(kEnqueueOp), pfq,
  ...
  auto w = Variable(root.WithOpName("w"), {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  ...
  auto ada_w = Variable(root.WithOpName("ada_w"), {INPUT_SIZE + NUM_UNIT, NUM_UNIT * 4}, DT_FLOAT);
  ...
  auto ada_w_y = Variable(root.WithOpName("ada_w_y"), {INPUT_SIZE, NUM_UNIT}, DT_FLOAT);
  ...
  auto x = Placeholder(root, DT_FLOAT, Placeholder::Shape({TIME_LEN, BATCH_SIZE, INPUT_SIZE}));
  // (timelen, batch_size)
  auto y = Placeholder(root, DT_INT64, Placeholder::Shape({TIME_LEN, BATCH_SIZE}));

  // (batch_size, cell_size)
  auto cs_prev = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT}));
  auto h_prev = Placeholder(root, DT_FLOAT, Placeholder::Shape({BATCH_SIZE, NUM_UNIT}));

  // LSTM
  auto block_lstm = BlockLSTM(root.WithOpName("block_lstm"),
  ...
  auto rnn_softmax_loss = RNNSoftmaxLoss(root.WithOpName("rnn_softmax_loss"),
                              block_lstm.h,
                              y,
                              w_y,
                              b_y,
      ...
      Tensor x_tensor(DT_FLOAT, TensorShape({TIME_LEN, BATCH_SIZE, INPUT_SIZE}));
      {
        ...
        for (int i = 0; i < TIME_LEN; i++) {
          ...
          Eigen::Tensor<float, 2, Eigen::RowMajor> mat
                = dataset_outputs[1].matrix<float>().slice(indices_dataset, sizes_dataset);
