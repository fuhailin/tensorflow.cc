...
static tensorflow::Status Iterator(const tensorflow::Scope& scope, 
  ...
  auto builder = tensorflow::NodeBuilder(unique_name, "Iterator")
                    ...
                    .Attr("container", "")
                    .Attr("output_types", std::vector<tensorflow::DataType>({tensorflow::DT_STRING})) // TFRecordDataset only support DT_STRING, this doesn't make sense
                    .Attr("output_shapes", std::vector<tensorflow::PartialTensorShape>({{}})); // TFRecordDataset only support {}, this doesn't make sense
...
static tensorflow::Status IteratorGetNext(const tensorflow::Scope& scope, 
  ...
  auto builder = tensorflow::NodeBuilder(unique_name, "IteratorGetNext")
                    .Input(_iterator)
                    .Attr("output_types", std::vector<tensorflow::DataType>({tensorflow::DT_STRING})) // 
...
static tensorflow::Tensor make_dense_default(tensorflow::DataType dtype, int feature_size) {
    return tensorflow::Tensor(dtype, tensorflow::TensorShape({feature_size}));
}

static tensorflow::Node* Constant(tensorflow::Graph* g, const tensorflow::Tensor& tensor) {
  tensorflow::Node* ret;
...
static tensorflow::Status ParseExample(const tensorflow::Scope& scope, 
  ...
  // Prepare inputs  
  tensorflow::Tensor names(tensorflow::DT_STRING, tensorflow::TensorShape({batch_size}));
  ...
  for (int i = 0; i < num_keys; ++i) {
    ...
    switch (benchmark_type) {
      case kDense:
        ...
        dense_defaults.emplace_back(Constant(
            scope.graph(), make_dense_default(dtype, feature_size)));
...
int main() {
  ...
  tensorflow::Tensor filenames("/tmp/test1.tfrecord"); // Check tfrecord_test.py which generates the tfrecord file
  ...
  if (!TFRecordDataset(root, filenames, compression_type, buffer_size, dataset_output).ok()) {
  ...
  if (!MakeIterator(root, dataset_output, iterator_output, make_iterator_output).ok()) {
    LOG(ERROR) << "-----------------------------------------status: " << root.status();
  ...
  TF_CHECK_OK(session.Run({}, 
                          {tensorflow::Output(parse_example_output.node(), 0), tensorflow::Output(parse_example_output.node(), 1), 
                                  tensorflow::Output(parse_example_output.node(), 2), tensorflow::Output(parse_example_output.node(), 3)}, 
                          {parse_example_output.op()}, 
                          &outputs));

  LOG(INFO) << "Print: " << outputs[0].shape() << ", " << outputs[1].shape() << ", " << outputs[2].shape() << ", " << outputs[3].shape();
  LOG(INFO) << "Print: " << outputs[0].DebugString() << outputs[1].DebugString() << outputs[2].DebugString() << outputs[3].DebugString();
